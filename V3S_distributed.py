...
class DistributedParticipant(threading.Thread):
    ...
    def __init__(self, participant_id: int, n: int, t: int, d: int, 
                 network: NetworkSimulator, shared_password: str,
    ...
    def receive_and_verify_shares(self):
        ...
        for package in encrypted_packages:
            try:
                ...
                print(f"[Participant {self.participant_id}] Failed to decrypt share from {package.sender_id}: {e}")
        ...
        print(f"[Participant {self.participant_id}] Verification complete: {verified_count} valid, {failed_count} invalid (out of {len(all_proofs)-1})")
        
        # 接收其他参与者的投诉
        print(f"[Participant {self.participant_id}] Listening for complaints...")
        time.sleep(0.5)  # 等待投诉广播
        ...
        if received_complaints:
            ...
            for complaint in received_complaints:
                ...
                # 如果被投诉者在我的有效份额数组中，将其移除
                if complaint.accused_id in self.valid_shares:
                    self.valid_shares.remove(complaint.accused_id)
                    print(f"[Participant {self.participant_id}] ⚠️  Removed Participant {complaint.accused_id} from valid shares (complained by Participant {complaint.complainer_id})")
    ...
    def aggregate_and_reconstruct_global_secret(self):
        ...
        for dim in range(self.d):
            ...
            for pid in selected_pids:
                share_value = available_agg_shares[pid][dim]
                shares_for_dim.append(Share(value=share_value, index=pid))
